...
def iou_3d(bbox1, bbox2):
    ...
    if (
        overlap_x2 - overlap_x1 <= 0
        or overlap_y2 - overlap_y1 <= 0
        or overlap_z2 - overlap_z1 <= 0
    ):
        return 0
...
def _compile_pred_metrics(iou_matrix, gt_info, pred_info):
    ...
    pred_metrics["hit_label"] = pred_metrics.apply(
        lambda x: x["hit_label"] if x["max_iou"] > 0 else 0, axis=1
    )
    ...
    pred_metrics.rename({"probability": "prob"}, axis=1, inplace=True)
    pred_metrics.drop("label_id", axis=1, inplace=True)

    pred_metrics = pred_metrics.merge(
        gt_info["label_id"], how="left", left_on="hit_label", right_on="label_id"
    )
    pred_metrics.drop("label_id", axis=1, inplace=True)
...
def evaluate_single_prediction(gt_info, pred_info):
    ...
    for gt_idx in range(num_gt):
        ...
        gt_bbox = [
            ...
            gt_bbox_info["coordY"],
            gt_bbox_info["coordZ"],
            gt_bbox_info["x_length"],
            gt_bbox_info["y_length"],
            gt_bbox_info["z_length"],
        ]
        ...
        for pred_idx in range(num_pred):
            ...
            pred_bbox = [
                pred_bbox_info["coordX"],
                pred_bbox_info["coordY"],
                pred_bbox_info["coordZ"],
                pred_bbox_info["x_length"],
                pred_bbox_info["y_length"],
                pred_bbox_info["z_length"],
            ]
...
def _froc_single_thresh(df_list, num_gts, p_thresh, iou_thresh):
    ...
    df_pos_pred = [df.loc[df["prob"] >= p_thresh] for df in df_list if len(df) > 0]

    # calculate total true positives
    total_tp = sum(
        [
            len(df.loc[df["max_iou"] > iou_thresh, "hit_label"].unique())
    ...
    total_fp = sum(
        [len(df) - len(df.loc[df["max_iou"] > iou_thresh]) for df in df_pos_pred]
    )
...
def _get_key_recall(fp, recall, key_fp_list):
    ...
    key_recall = [
        _interpolate_recall_at_fp(fp_recall, key_fp) for key_fp in key_fp_list
    ]
...
def froc(df_list, num_gts, iou_thresh=0.3, key_fp=DEFAULT_KEY_FP):
    ...
    """
    fp_recall = [
        _froc_single_thresh(df_list, num_gts, p_thresh, iou_thresh)
...
def evaluate(gt_csv_path, pred_csv_path):
    ...
    for i in pred_pids:
        assert i in gt_pids, "Unmatched seriesuid (not included in test set)."

    eval_results = []
    progress = tqdm(total=len(gt_pids))
    for pid in gt_pids:
        # get GT array and information
        cur_gt_info = gt_info.loc[gt_info.public_id == pid].reset_index(drop=True)
        cur_gt_info["label_id"] = np.arange(1, len(cur_gt_info) + 1)

        # get prediction array and information
        cur_pred_info = pred_info.loc[pred_info.public_id == pid].reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    eval_results = evaluate(
        ...
        os.path.join(
            ...
            "predict",
            "Detection",
            os.listdir(f"{team}/predict/Detection")[0],
        ),
    )
    ...
    froc_recall = pd.DataFrame(
        np.array(eval_results["detection"]["key_recall"]).reshape(1, -1),
        index=["Recall"],
        columns=[f"FP={str(x)}" for x in DEFAULT_KEY_FP],
    )
    print(froc_recall)
    print("Average recall: {:.4f}".format(eval_results["detection"]["average_recall"]))
    print("Maximum recall: {:.4f}".format(eval_results["detection"]["max_recall"]))
    print(
        "Average FP per scan at maximum recall: {:.4f}".format(
    ...
    # for fp, recall in zip(
    #     reversed(eval_results["detection"]["fp"]),
